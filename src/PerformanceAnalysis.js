...
/** Expected Format each recommendation as follows:
...
const PerformanceAnalysis = ({ data }) => {
    ...
    useEffect(() => {
              ...
              lines.forEach(line => {
                ...
                if (colonIndex !== -1) {
                  ...
                  if (key.toLowerCase().includes('steps')) {
                    ...
                    currentKey = 'Effort';
                  } else if (key.toLowerCase().includes('Potential performance benefits')) {
                    recommendation['Benefits'] = value;
                    currentKey = 'Benefits';
                  } else if (key.toLowerCase().includes('Performance implications')) {
        ...
        if (data.length === 0) {
          setError('No performance analysis data found');
        }else{
          setError('');
            if (data) {
   ...
   return (
    ...
    {parsedSections.length > 0 ? (
      ...
      <Card.Body>
        ...
        <Card.Text>
          ...
          <strong>Effort:</strong> {section.Effort || 'N/A'}<br />
            <strong>Potential security benefits:</strong> {section.Benefits || 'N/A'}<br />
            <strong> Security implications:</strong> {section.Effectiveness || 'N/A'}<br />
      ...
      <Modal show={showFeedbackModal} onHide={() => setShowFeedbackModal(false)}>
        ...
        <div>Loading performance analysis...</div>
      )}
    </div>
  );
};
