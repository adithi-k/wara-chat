...
const SecurityAnalysis = ({ data }) => {
    ...
    useEffect(() => {
              ...
              lines.forEach(line => {
                ...
                if (colonIndex !== -1) {
                  ...
                  if (key.toLowerCase().includes('steps')) {
                    ...
                    currentKey = 'Effort';
                  } else if (key.toLowerCase().includes('Potential security benefits')) {
                    recommendation['Benefits'] = value;
                    currentKey = 'Benefits';
                  } else if (key.toLowerCase().includes('Security implications')) {
        ...
        if (data.length === 0) {
          setError('No security analysis data found');
        }else{
          setError('');
            if (data) {
